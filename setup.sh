...
run_step() {
    ...
    # Prompt for key press before executing the command (only when running interactively)
    if [[ -t 0 ]]; then
        read -n1 -s -r -p "Press any key to execute this step..."
        echo
    fi
...
run_step "Allow HTTP and HTTPS traffic through the firewall..." "apt install ufw -y && ufw --force enable && ufw allow 'Apache Full' && ufw allow ssh"
...
run_step "Set up public_html directories for users..." "mkdir -p /home/linux1/public_html/cgi-bin /home/linux2/public_html/cgi-bin && chmod 755 /home/*/public_html"
run_step "Enable CGI execution in user directories..." "printf '<Directory /home/*/public_html>\n    Options Indexes FollowSymLinks\n    Require all granted\n</Directory>\n<Directory /home/*/public_html/cgi-bin>\n    Options +ExecCGI -Indexes\n    AddHandler cgi-script .py\n    Require all granted\n</Directory>\nUserDir public_html\n' | tee /etc/apache2/mods-available/userdir.conf > /dev/null && a2enmod userdir && systemctl restart apache2"
...
run_step "Set up proxy for router admin interface..." "printf '<Location /router>\n    ProxyPass http://192.168.1.1:80\n    ProxyPassReverse http://192.168.1.1:80\n    Require all granted\n</Location>' | tee /etc/apache2/sites-available/router-proxy.conf && a2ensite router-proxy && systemctl reload apache2"
...
# already enabled - run_step "Set up compression for text-based content..." "echo 'AddOutputFilterByType DEFLATE text/html text/plain text/xml text/css text/javascript application/javascript' | tee -a /etc/apache2/mods-available/deflate.conf && systemctl restart apache2"
...
run_step "Configure .htaccess permissions..." "printf '<Directory /var/www/>\n    AllowOverride All\n</Directory>\n<Directory /home/*/public_html/>\n    AllowOverride All\n</Directory>' | tee /etc/apache2/conf-available/htaccess.conf && a2enconf htaccess && systemctl reload apache2"
